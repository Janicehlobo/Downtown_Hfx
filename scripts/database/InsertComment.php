...
try {
  ...
  session_start();
   ...
   if($_POST){
    ...
    $AdminComment = $_POST['admCommentForUsers'];
    $createdate = date('Y-m-d');
   
 $sql  = "INSERT INTO commenttable (comment,createdAt,user_iduser) values (:AdminComment ,:createdate, :userid)" ;     

  
   $stmt = $conn->prepare($sql);
   $stmt->execute(array('AdminComment' => $AdminComment, 'createdate' => $createdate, 'userid' => $userid ));
