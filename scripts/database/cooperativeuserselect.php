...
try {
	...
	if (isset($_SESSION["userlogin"])){
		...
		$_SESSION['fyearuco']= $_POST['fromyearuco'];
		$_SESSION['tyearuco']=$_POST['toyearuco'];
	  }
	
	$sql = ("SELECT cooptable.industry, cooptable.type, cooptable.language, datatable.year, datatable.assetSize, datatable.revenueSize, kpitable.title, kpivalue.kpiValue 
	from user, access,cooptable, datatable, kpitable, kpivalue 
	where user.iduser = '$userid' && user.iduser = access.user_iduser && access.cooptable_idcoop = cooptable.idcoop && cooptable.idcoop = datatable.cooptable_idcoop && datatable.iddata = kpivalue.datatable_iddata && kpitable.idkpi = kpivalue.kpitable_idkpi && datatable.year BETWEEN :fyear AND :tyear
