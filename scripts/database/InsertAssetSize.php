...
try {
  ...
  session_start();
   ...
   if($_POST){
    ...
    $AssValue = $_POST['assetValue'];       
    $createdate = date('Y-m-d');

    $sql  = "INSERT INTO dropdowntable (dropdownName,dropdownValue,createdAt,createdBy,dstatus) values ('AssetSize', :AssValue , :createdate, :userid ,'Active')" ;
  
   $stmt = $conn->prepare($sql);
   $stmt->execute(array('AssValue' => $AssValue , 'createdate' => $createdate , 'userid' => $userid));
