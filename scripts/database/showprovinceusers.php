...
try {
	...
	$fyear = $_POST['fromyearup'];
    $tyear = $_POST['toyearup'];
	if (isset($_SESSION["userlogin"])){
        $userid = $_SESSION['userlogin'];
        $_SESSION['fyearupa']= $_POST['fromyearup'];
		$_SESSION['tyearupa']=$_POST['toyearup'];
	  }
	
    $sql = ("SELECT cooptable.industry, cooptable.province, cooptable.language, datatable.year, datatable.assetSize, datatable.revenueSize, kpitable.title, kpivalue.kpiValue from user, access,cooptable, datatable, kpitable, kpivalue where (user.iduser = access.user_iduser && access.cooptable_idcoop = cooptable.idcoop && cooptable.idcoop = datatable.cooptable_idcoop && kpitable.idkpi=kpivalue.kpitable_idkpi && datatable.iddata= kpivalue.datatable_iddata && datatable.year BETWEEN :fyear AND :tyear && cooptable.province = (select cooptable.province from user, access, cooptable where user.iduser = :userid 
    && user.iduser = access.user_iduser && access.cooptable_idcoop = cooptable.idcoop) && (select count(distinct user.iduser) from user, access, cooptable, datatable where user.iduser = access.user_iduser && access.cooptable_idcoop = cooptable.idcoop && cooptable.province = (select cooptable.province from user, access, cooptable where user.iduser = '$userid' && user.iduser = access.user_iduser && access.cooptable_idcoop = cooptable.idcoop)) >2) order by user.iduser");
