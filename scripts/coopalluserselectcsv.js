...
$(document).ready(function() {
    var link = $("#userallcooperative");
	$("#usercooplink15").on("click", function() {
	$.post("scripts/database/showcooperativeuserscsv.php", null, function(data){
      // Change the data from plain text that PHP echo'd to a JavaScript object
		...
		csv += industry + ',' + type + ',' + language + ',' + year + ',' + assetSize + ',' + revenueSize + ',' + kpiname + ',' + kpiValue + '\n';
        }			
        console.log(csv);
        link.attr("href",'data:application/csv;charset=UTF-8,' + encodeURI(csv));
		link.attr("download" ,"All-user-industry.csv");
        link[0].click();
    });
	});
	})